...
local update_widget = function(widget, stdout, stderr)
	...
	local avg = (result.daily[1].temp.day + result.daily[1].temp.night + result.daily[1].temp.eve + result.daily[1].temp.morn) / 4
    temperature.markup = "<span foreground='"..getColorFromTemp(result.daily[1].temp.max).."'>"..result.daily[1].temp.max.." °C</span>"
    description.markup = "<span foreground='"..beautiful.fg_normal.."'>"..result.daily[1].weather[1].description.."</span>" 
    
    local condition = result.daily[1].weather[1].main
